...
config() {
    ...
    case $? in
        0)
            echo -e "JvpnXR状态: ${green}已运行${plain}"
            ;;
        1)
            echo -e "检测到您未启动JvpnXR或JvpnXR自动重启失败，是否查看日志？[Y/n]" && echo
            read -e -p "(默认: y):" yn
            [[ -z ${yn} ]] && yn="y"
            if [[ ${yn} == [Yy] ]]; then
               show_log
            ...
            ;;
        2)
            echo -e "JvpnXR状态: ${red}未安装${plain}"
    esac
}

uninstall() {
    confirm "确定要卸载 JvpnXR 吗?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    echo ""
    echo -e "卸载成功，如果你想删除此脚本，则退出脚本后运行 ${green}rm /usr/bin/JvpnXR -f${plain} 进行删除"
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}JvpnXR已运行，无需再次启动，如需重启请选择重启${plain}"
    else
        systemctl start JvpnXR
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}JvpnXR 启动成功，请使用 JvpnXR log 查看运行日志${plain}"
        else
            echo -e "${red}JvpnXR可能启动失败，请稍后使用 JvpnXR log 查看日志信息${plain}"
        fi
    fi
...
stop() {
    ...
    check_status
    if [[ $? == 1 ]]; then
        echo -e "${green}JvpnXR 停止成功${plain}"
    else
        echo -e "${red}JvpnXR停止失败，可能是因为停止时间超过了两秒，请稍后查看日志信息${plain}"
    fi
...
restart() {
    systemctl restart JvpnXR
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}JvpnXR 重启成功，请使用 JvpnXR log 查看运行日志${plain}"
    else
        echo -e "${red}JvpnXR可能启动失败，请稍后使用 JvpnXR log 查看日志信息${plain}"
    fi
    if [[ $# == 0 ]]; then
...
status() {
    systemctl status JvpnXR --no-pager -l
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}JvpnXR 设置开机自启成功${plain}"
    else
        echo -e "${red}JvpnXR 设置开机自启失败${plain}"
    fi
...
disable() {
    systemctl disable JvpnXR
    if [[ $? == 0 ]]; then
        echo -e "${green}JvpnXR 取消开机自启成功${plain}"
    else
        echo -e "${red}JvpnXR 取消开机自启失败${plain}"
    fi
...
show_log() {
    journalctl -u JvpnXR.service -e --no-pager -f
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
check_enabled() {
    temp=$(systemctl is-enabled JvpnXR)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}JvpnXR已安装，请不要重复安装${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
show_status() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "JvpnXR状态: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
            echo -e "JvpnXR状态: ${red}未安装${plain}"
...
show_JvpnXR_version() {
    echo -n "JvpnXR 版本："
    /usr/local/JvpnXR/JvpnXR -version
    echo ""
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
————————————————
  ...
  ${green}2.${plain} 更新 JvpnXR
  ${green}3.${plain} 卸载 JvpnXR
————————————————
  ${green}4.${plain} 启动 JvpnXR
  ${green}5.${plain} 停止 JvpnXR
  ${green}6.${plain} 重启 JvpnXR
  ${green}7.${plain} 查看 JvpnXR 状态
  ${green}8.${plain} 查看 JvpnXR 日志
————————————————
  ${green}9.${plain} 设置 JvpnXR 开机自启
 ${green}10.${plain} 取消 JvpnXR 开机自启
————————————————
 ${green}11.${plain} 一键安装 bbr (最新内核)
 ${green}12.${plain} 查看 JvpnXR 版本 
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) install_bbr
        ;;
        12) check_install && show_JvpnXR_version
        ;;
        13) update_shell
        ;;
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        ;;
        "version") check_install 0 && show_JvpnXR_version 0
        ;;
